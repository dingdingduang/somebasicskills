...
//        int topPosition = PADDING + 10;
    ...
    public void render(PoseStack pPoseStack, int mouseX, int mouseY, float partialTick) {
        ...
        // Title
        GuiMethods.drawString(pPoseStack, this.font, this.title.getString(), this.listWidth + 10 + 10, 8, 0xFFFFFF);
...
//                            k = (tempIntI + 90) % width;
        ...
        if (this.showSkillTree()) {
          this.showSkillTreeScreen.render(pPoseStack, mouseX, mouseY, partialTick);
        }

        if (this.showSBSettingInfoScreen()) {
            this.LocalSBSettingInfoScreen.render(pPoseStack, mouseX, mouseY, partialTick);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scroll) {
        if (this.showSkillTree()) {
            this.showSkillTreeScreen.mouseScrolled(mouseX, mouseY, scroll);
        }

        if (this.showSBSettingInfoScreen()) {
            this.LocalSBSettingInfoScreen.mouseScrolled(mouseX, mouseY, scroll);
        }
