...
//        this.IncrementTextLengthVal = (int) (this.MaxTextLength * 0.1666667f);
    ...
    public void render(PoseStack pPoseStack, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.showSBSettingDetailsScreen()) {
            this.LocalSBSettingDetailsScreen.render(pPoseStack, mouseX, mouseY, partialTick);
        }
        else {
    ...
    public void renderAllSettingBtnsDescription(PoseStack pPoseStack) {
        ...
        for (String configName : tempClientConfigSetting.keySet()) {
            ...
            if (tempSeparatedStrArrList.size() > 1) {
                ...
                GuiMethods.drawString(pPoseStack, this.font, tempStrBuilder1.toString(), 32, tempImgBtn.getSBSBtnPosY() + 4, 0xFFFFFF);
                tempImgBtn.setButtonOriginalPosY( currentUpdatedBtnPos );
                tempImgBtn.setSBSBtnPosY(tempImgBtn.getButtonOriginalPosY());
            }
        }
