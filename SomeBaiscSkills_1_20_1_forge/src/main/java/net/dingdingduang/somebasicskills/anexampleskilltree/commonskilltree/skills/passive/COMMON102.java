...
public class COMMON102 {
    ...
    public static void init() {
        SkillDataJson tempSkillData = getID2SkillData().get(CommonSkillInitialization.COMMON_102_CLOTH_ARMOR_MASTERY);
        tempSkillData.setTranslatableTooltipGetter(COMMON102::getSkillDescription);

        MethodAction tempAct =
                (entity1) -> {
                    double amount = 0.0;
                    int playerSkillLVL = 0;
                    SkillDataJson skill1 = getID2SkillData().get(CommonSkillInitialization.COMMON_102_CLOTH_ARMOR_MASTERY);
                    if (entity1 instanceof ServerPlayer sp1) {
                        if (getGlobalPlayerSkillID2lvlMap().get(sp1).containsKey(CommonSkillInitialization.COMMON_102_CLOTH_ARMOR_MASTERY)) {
                            playerSkillLVL = getGlobalPlayerSkillID2lvlMap().get(sp1).get(CommonSkillInitialization.COMMON_102_CLOTH_ARMOR_MASTERY);
                    ...
                    PermAttributesDistributor.applyPermArmorToEntity(entity1, amount, Constants.OP_ADDITION, CommonSkillInitialization.COMMON_102_CLOTH_ARMOR_MASTERY, null);
                };
