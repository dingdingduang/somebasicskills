...
public class COMMON103 {
    ...
    public static void init() {
        SkillDataJson tempSkillData = getID2SkillData().get(CommonSkillInitialization.COMMON_103_HEAVY_ARMOR_MASTERY);
        tempSkillData.setTranslatableTooltipGetter(COMMON103::getSkillDescription);

        MethodAction tempAct =
                (entity1) -> {
                    double amount = 0.0;
                    int playerSkillLVL = 0;
                    SkillDataJson skill1 = getID2SkillData().get(CommonSkillInitialization.COMMON_103_HEAVY_ARMOR_MASTERY);
                    if (entity1 instanceof ServerPlayerEntity sp1) {
                        if (getGlobalPlayerSkillID2lvlMap().get(sp1).containsKey(CommonSkillInitialization.COMMON_103_HEAVY_ARMOR_MASTERY)) {
                            playerSkillLVL = getGlobalPlayerSkillID2lvlMap().get(sp1).get(CommonSkillInitialization.COMMON_103_HEAVY_ARMOR_MASTERY);
                    ...
                    PermAttributesDistributor.applyPermArmorToEntity(entity1, amount, Constants.OP_ADDITION, CommonSkillInitialization.COMMON_103_HEAVY_ARMOR_MASTERY, null);
                };
