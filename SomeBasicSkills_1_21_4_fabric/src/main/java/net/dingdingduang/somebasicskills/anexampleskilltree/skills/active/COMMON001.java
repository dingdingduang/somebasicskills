...
//            getCPlayerState().put(Constants.IS_BACKSTEPPING, Constants.ACTION_ON);
        ...
        MethodAction tempSkillAction =
                (entity1) -> {
                    ...
                    PlaySoundAtLocation(COMMON_0_1_LEAP_SOUND, entity1.getWorld(), entity1.getX(), entity1.getY(), entity1.getZ(), 0.5f, 1.0f);
                    ...
                    MethodAction action =
                            (entity2) ->  {
                                ...
                                if (distHasTraveled < finalDist || updatedTick > 60) {
                                    ...
                                    //check collision
                                    int x = (int) Math.floor(entity2.getX() + currentDistShouldTravel*Math.sin((-entity2.getYaw()+180) * Math.PI / 180.0));
                                    int y = (int) Math.floor(entity2.getY());
                                    int z = (int) Math.floor(entity2.getZ() + currentDistShouldTravel*Math.cos((-entity2.getYaw()+180) * Math.PI / 180.0));
                                    BlockState state = entity2.getWorld().getBlockState(new BlockPos(x, y, z));

                                    if (!entity2.isOnGround() && !state.isAir()) {
