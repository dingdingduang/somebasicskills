...
public abstract class ScrollPanel extends AbstractParentElement implements Drawable, Selectable {
    ...
    protected void drawBackground(DrawContext drawContext, MatrixStack matrix, Tessellator tessellatorellator, float partialTick) {
        if (getClientWorld() != null) {
            GuiMethods.guiUtilsFillGradient(drawContext, this.left, this.top, this.right, this.bottom, this.bgColorFrom, this.bgColorTo, 0);
        }
        else {
            RenderSystem.setShader(ShaderProgramKeys.POSITION_TEX_COLOR);
            RenderSystem.setShaderTexture(0, IconBasicResourceLocation.SKILL_BACKGROUND);

            BufferBuilder bufferBuilder = tessellatorellator.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
            bufferBuilder.vertex(this.left, this.bottom, 0).texture(this.left / 32F, (this.bottom + (int) this.scrollDistance) / 32F).color(32, 32, 32, 255);
            bufferBuilder.vertex(this.right, this.bottom, 0).texture(this.right / 32F, (this.bottom + (int) this.scrollDistance) / 32F).color(32, 32, 32, 255);
            bufferBuilder.vertex(this.right, this.top, 0).texture(this.right / 32F, (this.top + (int) this.scrollDistance) / 32F).color(32, 32, 32, 255);
            bufferBuilder.vertex(this.left, this.top, 0).texture(this.left / 32F, (this.top + (int) this.scrollDistance) / 32F).color(32, 32, 32, 255);
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float partialTick) {
        ...
        this.drawBackground(drawContext, matrix, tessellator, partialTick);
...
//            RenderSystem.disableTexture();
            ...
            bufferBuilder.vertex(this.barLeft, this.bottom, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth), this.bottom, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth), this.top, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha);
            bufferBuilder.vertex(this.barLeft, this.top, 0.0F).color(barBgRed, barBgGreen, barBgBlue, barBgAlpha);
            BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());
            ...
            bufferBuilder.vertex(this.barLeft, (barTop + barHeight), 0.0F).color(barRed, barGreen, barBlue, barAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth), (barTop + barHeight), 0.0F).color(barRed, barGreen, barBlue, barAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth), barTop, 0.0F).color(barRed, barGreen, barBlue, barAlpha);
            bufferBuilder.vertex(this.barLeft, barTop, 0.0F).color(barRed, barGreen, barBlue, barAlpha);
            BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());
            ...
            bufferBuilder = tessellator.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
            bufferBuilder.vertex(this.barLeft, (barTop + barHeight - 1), 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth - 1), (barTop + barHeight - 1), 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha);
            bufferBuilder.vertex((this.barLeft + this.barWidth - 1), barTop, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha);
            bufferBuilder.vertex(this.barLeft, barTop, 0.0F).color(barBorderRed, barBorderGreen, barBorderBlue, barBorderAlpha);
            BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());
        }
