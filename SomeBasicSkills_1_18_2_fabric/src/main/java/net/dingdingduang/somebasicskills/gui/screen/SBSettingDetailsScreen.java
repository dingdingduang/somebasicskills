...
public class SBSettingDetailsScreen extends Screen {
    ...
    protected SBSettingDetailsScreen(Text component) { super(component); }
...
//        this.IncrementTextLengthVal = (int) (this.MaxTextLength * 0.1666667f);
    ...
    @Override
    public void render(MatrixStack pPoseStack, int mouseX, int mouseY, float partialTick) {
        ...
        for (String configName : tempBtn2Setting.keySet()) {
            ...
            if (tempSeparatedStrArrList.size() > 1) {
                ...
                if (configHelper.isBooleanConfig()) {
                    ...
                    GuiMethods.drawString(pPoseStack, getFont(), tempStrBuilder1.toString(), this.IncrementTextLengthVal * 6 + 38, currentUpdatedBtnPos + 4 + 4 * extraYCounter, 0xFFFFFF);
                }
                else {
...
//                    GuiMethods.drawString();(pPoseStack, getFont(), tempStrBuilder1.toString(), 32, this.ConfigName2SettingBtn.get(configName).getY() + 4, 0xFFFFFF);
            }
            else {
                tempStrBuilder1 = new StringBuilder();
                tempStrBuilder1.append(tempSeparatedStrArrList.get(0));
                ...
                GuiMethods.drawString(pPoseStack, getFont(), tempStrBuilder1.toString(), 32, tempImgBtn.getButtonOriginalPosY() + 4, 0xFFFFFF);
                tempImgBtn.setButtonOriginalPosY( currentUpdatedBtnPos );
                tempImgBtn.setSBSBtnPosY(tempImgBtn.getButtonOriginalPosY());
            }
        }
    ...
    public void renderBackground(MatrixStack pPoseStack) {
        if (getMinecraftInstance() != null && getMinecraftInstance().world != null) {
