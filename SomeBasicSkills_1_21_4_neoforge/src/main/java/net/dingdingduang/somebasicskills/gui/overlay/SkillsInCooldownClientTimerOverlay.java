...
public class SkillsInCooldownClientTimerOverlay implements LayeredDraw.Layer {
...
//    private final int SkillIconFrameSize = 16;
    ...
    public void drawSquareImage(GuiGraphics guiGraphics, int x, int y, int translateZ, ResourceLocation res1, int size) {
        ...
        guiGraphics.blit(RenderType::guiTextured, res1, x, y, size, size, size, size, size, size);
    ...
    public void drawCenterString(GuiGraphics guiGraphics, int x1, int y1, int translateZ, String text, int pColor) {
        ...
        guiGraphics.drawCenteredString(getMinecraftInstanceFont(), text, x1, y1, pColor);

        guiGraphics.pose().popPose();
    }

    public void renderSkillIcons(GuiGraphics guiGraphics, int screenWidth, int screenHeight, SkillDataJson skill1, int skillTickElapsed, double skillCooldownTime, int tempSkillIndex) {
        if (getRole2IconLocMap().containsKey(skill1.getRoleCategoryName())) {
            ...
            drawSquareImage(guiGraphics, posX, posY- tempInterval, 189, IconBasicResourceLocation.ICON_FRAME_BACKGROUND, SkillIconFrameSize);
            if (CooldownPercent < 0.05 && skillTickElapsed % 2 == 0) {
                drawSquareImage(guiGraphics, posX+1, posY-tempInterval +1, 190, getRole2IconLocMap().get(skill1.getRoleCategoryName()).get(skill1.getSkillID()), SkillIconSize);
            }
            else {
                drawSquareImage(guiGraphics, posX+1, posY-tempInterval +1, 190, getRole2IconLocMap().get(skill1.getRoleCategoryName()).get("DISBTN"+skill1.getSkillID()), SkillIconSize);
            }

            fillGradient(guiGraphics, posX+1, posY-tempInterval+1, posX+ SkillIconFrameSize -1, posY-tempInterval-1 + ((int) Math.round( SkillIconFrameSize * CooldownPercent ) ), 191, -0x50FFEFF0);

            if (showCooldown) {
                int posX_center = (posX + (posX + SkillIconFrameSize)) / 2;
                int posY_center = ((posY - tempInterval) + (posY - tempInterval + SkillIconFrameSize)) / 2;
                drawCenterString(guiGraphics, posX_center, posY_center + 9, 192, "" + (Math.round((skillCooldownTime - skillTickElapsed) / 20 * 10) * 1.0 / 10), 0xFFFFFF);
//                drawCenterString(guiGraphics, posX_center, posY_center + 9, 192, "" + (Math.round((skillCooldownTime - skillTickElapsed) / 20 * 100) * 1.0 / 100), 0xFFFFFF);
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        if (getMinecraftInstance().player != null) {
            renderMore(getMinecraftInstance().player.tickCount, guiGraphics, guiGraphics.guiWidth(), guiGraphics.guiHeight());
        }
    }

    public void renderMore(int currentGameTicks, GuiGraphics guiGraphics, int screenWidth, int screenHeight) {
            ...
            for (String skillID: this.PlayerSkill2CooldownTicks.keySet()) {
                ...
                if (skillTickElapsed > skillCooldownTime) {
                    ...
                    if (isIconVisible) {
                        ...
                        renderSkillIcons(guiGraphics, screenWidth, screenHeight, skill1, skillTickElapsed, skillCooldownTime, tempSkillIndex);
                    }
