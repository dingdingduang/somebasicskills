...
public class SkillInfoScreen extends Screen {
    private static final ResourceLocation MC_DEFAULT_WINDOW_BAKCGROUND = getMCResourceLocation(Constants.MOD_ID, "textures/gui/window.png");
    ...
    private Player triggeredLocalPlayer;
...
//    public SkillInfoScreen(String title, String skillDescription, String roleCategory, String skillID, Player player) {
    public SkillInfoScreen(String title, String roleCategory, String skillID, Player player) {
...
//        this.description = getLocalizationText(skillDescription);
    ...
    protected SkillInfoScreen(Component component) {
        super(component);
    }
...
//            info.add("Required Player XP Level:");
...
//        printInGameMsg(info.toString());
    ...
    public void init() {
        ...
        this.skillInfoPanel = new SkillInfoWidget(minecraft, maxWidth - 18, maxHeight - 38 - 48, top + 18 + 48, left + 3);
        this.skillInfoPanel.setInfo(prepareInfoContent());
        this.addRenderableWidget(this.skillInfoPanel);
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.skillInfoPanel != null) {
            this.skillInfoPanel.render(guiGraphics, mouseX, mouseY, partialTick);

            renderSkillButtonBG(guiGraphics);
            renderSkillButtons(guiGraphics);

            //render text
            //      renderSkillLevelText(guiGraphics, "LV"+Math.round(Math.random()*100), left+4+14+12, top+35+18, 0xFFFFFF);
            if (this.triggeredLocalPlayerSkillLVLMap.containsKey(this.SkillID)) {
                renderSkillLevelText(guiGraphics, "LV"+this.triggeredLocalPlayerSkillLVLMap.get(this.SkillID), left + 4 + 14 + 12, top + 35 + 18, 0xFFFFFF);
            }
            else {
                renderSkillLevelText(guiGraphics, "LV0", left + 4 + 14 + 12, top + 35 + 18, 0xFFFFFF);
            }
            //show xp
            renderSkillLevelText(guiGraphics, String.valueOf(SkillData.getXpCostPerLeveling()), left + 4 + 14 + 12+96, top + 35 + 15, 0xFFFFFF);

            if (!this.SkillData.isPassiveType()) {
                //skill priority
                renderSkillPriorityBtn(guiGraphics);
                //skill cooldown delay
                renderSkillCooldownDelayBtn(guiGraphics);

                //keyboard
                renderKeyboardBtns(guiGraphics);
                renderKeyboardText(guiGraphics);
            }
        }
    ...
    public void renderBackground(GuiGraphics guiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        guiGraphics.blit(RenderType::guiTextured, MC_DEFAULT_WINDOW_BAKCGROUND, left, top, 0, 0, maxWidth, heightPerPart, 256, 256);
...
//                left + 9, top + 18, left + maxWidth - 9, top + maxHeight - 20, -0x80FFEFF0, -0x80FFEFF0);
        ...
        // Title
        guiGraphics.drawString(this.font, this.title, left + 6, top + 6, 0xFFFFFF);
    ...
    public void renderSkillPriorityBtn(GuiGraphics guiGraphics) {
        if (this.isntSkillPrioritySettingBtnRendered) {
            addRenderableWidget(this.SkillPrioritySettingBtn);
            this.isntSkillPrioritySettingBtnRendered = false;
        }
    }

    public void renderSkillCooldownDelayBtn(GuiGraphics guiGraphics) {
        if (this.isntSkillCooldownDelaySettingBtnRendered) {
            addRenderableWidget(this.SkillCooldownDelaySettingBtn);
            this.isntSkillCooldownDelaySettingBtnRendered = false;
        }
    }

    public void renderKeyboardBtns(GuiGraphics guiGraphics) {
        if (this.isntKeyboardQuickSlotBtnRendered) {
            addRenderableWidget(this.KeyboardQuickSlotBtn);
            this.isntKeyboardQuickSlotBtnRendered = false;
        }
        if (this.isntKeyComboSettingBtnRendered) {
            addRenderableWidget(this.KeyComboSettingBtn);
            this.isntKeyComboSettingBtnRendered = false;
        }
    }

    public void renderSkillButtonBG(GuiGraphics guiGraphics) {
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0, 0, 202);

        drawImageOnly(guiGraphics, IconBasicResourceLocation.ICON_BG_EXPAND, left+12, top+18, 140, 48);

        guiGraphics.pose().popPose();
    }

    public void renderSkillButtons(GuiGraphics guiGraphics) {
        guiGraphics.pose().pushPose();
...
//        if (this.isBtnGray) { drawImageOnly(guiGraphics, getRole2IconLocMap().get(this.RoleCategory).get(Constants.GRAY_BTN_ICON_PREFIX + this.SkillID), left+4+12, top+3+18, 28, 28);}
        if (getClientPlayerXpLevel(this.triggeredLocalPlayer) < this.SkillData.getRequiredLevel()) { drawImageOnly(guiGraphics, getRole2IconLocMap().get(this.RoleCategory).get(Constants.GRAY_BTN_ICON_PREFIX + this.SkillID), left+4+12, top+3+18, 28, 28);}
        else { drawImageOnly(guiGraphics, getRole2IconLocMap().get(this.RoleCategory).get(this.SkillID), left+4+12, top+3+18, 28, 28); }

        //check conditions
//        if (!this.isBtnMeetRequirements) { renderSkillButtonOverlay(guiGraphics); }
        if (isntSatisfiedLevelRequirement()) {
            renderSkillButtonOverlay(guiGraphics);
        ...
        if (this.isAddBtnGray) {
            drawImageOnly(guiGraphics, IconBasicResourceLocation.DISICON_ADD, left+34+12, top+26+18, 18, 18);
            ...
            if (this.isntAddBtnRendered) {
                addRenderableWidget(this.IMGbuttonAdd);
                this.isntAddBtnRendered = false;
            }
        }
        if (this.isSubBtnGray) {
            drawImageOnly(guiGraphics, IconBasicResourceLocation.DISICON_SUB, left+34+18+12, top+26+18, 18, 18);
            ...
            if (this.isntSubBtnRendered) {
                addRenderableWidget(this.IMGbuttonSub);
                this.isntSubBtnRendered = false;
            }
        }
        if (this.isMaxBtnGray) {
            drawImageOnly(guiGraphics, IconBasicResourceLocation.DISICON_MAX, left+34+18*2+12, top+26+18, 18, 18);
            ...
            if (this.isntMaxBtnRendered) {
                addRenderableWidget(this.IMGbuttonMax);
    ...
    public void renderSkillButtonOverlay(GuiGraphics guiGraphics) {
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0, 0, 204);

        RenderSystem.enableBlend();
        drawImageOnly(guiGraphics, IconBasicResourceLocation.ICON_OVERLAY, left+4+12, top+3+18, 28, 28);
        RenderSystem.disableBlend();

        guiGraphics.pose().popPose();
    }

    public void renderSkillLevelText(GuiGraphics guiGraphics, String text, int x, int y, int pColor) {
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0, 0, 205);

//        guiGraphics.drawString(this.font, text, x, y, pColor);
        guiGraphics.drawCenteredString(this.font, text, x, y, pColor);
...
//            printInGameMsg("keycombo list: "+tempPlayerKeyCombo2SkillID.get(this.triggeredLocalPlayer));
    ...
    public void renderKeyboardText(GuiGraphics guiGraphics) {
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(0, 0, 201);
        ...
        guiGraphics.drawString(this.font, (this.SkillPrioritySettingText == null) ? "0": this.SkillPrioritySettingText, left + 172, top + 24, 0xFFFFFF);
        if (this.isntSkillPrioritySettingAssignedYet) {
            guiGraphics.drawString(this.font, (this.SkillCooldownDelaySettingText == null) ? "0.0" : this.SkillCooldownDelaySettingText, left + 208, top + 24, 0xFFFFFF);
        }
        guiGraphics.drawString(this.font, (this.SkillQuickslot == null) ? "N/A": this.SkillQuickslot, left + 172, top + 41, 0xFFFFFF);
        guiGraphics.drawString(this.font, (this.SkillKeyComb == null) ? "N/A": this.SkillKeyComb, left + 172, top + 58, 0xFFFFFF);
