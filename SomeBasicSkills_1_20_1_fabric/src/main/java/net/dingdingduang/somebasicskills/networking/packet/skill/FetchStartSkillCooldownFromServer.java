...
public class FetchStartSkillCooldownFromServer {
    public static Identifier ID = getMCResourceLocation(Constants.MOD_ID, "fetch_start_skill_cooldown_from_server");

    public static PacketByteBuf setupPacket(boolean isTimerOn, String skillID) {
    ...
    public static void toClientHandle(MinecraftClient client, ClientPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        client.execute( () -> {
            //start action
            SkillsInCooldownClientTimerOverlay.getSkillsInCooldownClientTimerOverlayInstance().setCooldownTimer(IsTimerOn, SkillID);
        });
    }
}
