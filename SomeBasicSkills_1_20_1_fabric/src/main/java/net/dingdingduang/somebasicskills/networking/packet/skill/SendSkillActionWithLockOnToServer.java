...
public class SendSkillActionWithLockOnToServer {
    public static Identifier ID = getMCResourceLocation(Constants.MOD_ID, "send_skill_action_with_lock_on_to_server");

    public static PacketByteBuf setupPacket(String skillID, float clientUserSkillIDDefaultSkillCDTime, float serverPlayerFacingAngle) {
        ...
        packetToBeSent.writeFloat(serverPlayerFacingAngle);
    ...
    public static void toServerHandle(MinecraftServer server, ServerPlayerEntity serverPlayer, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        final float ServerPlayerFacingAngle = buf.readFloat();
        ...
        server.execute( () -> {
            ...
            if (ClientUserSkillIDDefaultSkillCDTime >= ServerSkillIDDefaultCDTime - 1f) {
                if (skill1.isActiveType() && skill1.getActiveSkillAction1() != null) {
                    serverPlayer.setYaw(ServerPlayerFacingAngle);
