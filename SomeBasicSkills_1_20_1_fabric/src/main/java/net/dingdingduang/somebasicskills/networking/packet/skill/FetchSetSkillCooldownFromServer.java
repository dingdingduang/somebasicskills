...
public class FetchSetSkillCooldownFromServer {
    public static Identifier ID = getMCResourceLocation(Constants.MOD_ID, "fetch_set_skill_cooldown_from_server");
    ...
    public static void toClientHandle(MinecraftClient client, ClientPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        client.execute( () -> {
            //start action
            SkillsInCooldownClientTimerOverlay.getSkillsInCooldownClientTimerOverlayInstance().setCooldownTimer(IsTimerOn, SkillID, CooldownAmount);
        });
    }
}
